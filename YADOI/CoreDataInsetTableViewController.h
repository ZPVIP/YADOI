...
// 如果不能直接继承 UITableViewController 的话（例如要求 TableView 是 view 的一个 subView 时），就继承这个类
// 但是需要注意的是由于不是 UITableViewController，因此需要自己把 tableView 的 IBOutlet 连起来，然后设置tableView
// 的 DataSource 和 Delegate.
@interface CoreDataInsetTableViewController : UIViewController<UITableViewDataSource, UITableViewDelegate, NSFetchedResultsControllerDelegate>
